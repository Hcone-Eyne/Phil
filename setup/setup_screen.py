...
from setup.installer import install_ollama, pull_model, verify_ollama
...
class SetupScreen(ctk.CTk):
    def __init__(self, on_complete: Callable | None = None):
        ...
        self._selected_local_model: str = "qwen2.5-coder:3b"
        self._installed_models: list[str] = []
        self._ollama_installed: bool = False
        self._ollama_server_running: bool = False
        self._ollama_status_detail: str = ""
    ...
    def _run_checks(self):
        def update_status(msg: str):
            self.after(0, lambda: self._loading_sub.configure(text=msg))
        ...
        self._ollama_status_detail = getattr(ollama_result, "detail", "")
        ollama_value = getattr(ollama_result, "value", {}) or {}
        if isinstance(ollama_value, dict):
            self._ollama_server_running = bool(ollama_value.get("server_running", False))
            self._installed_models = ollama_value.get("models", [])
        else:
            self._ollama_server_running = False
        ...
        self._selected_local_model = self._recommended_model
    ...
    def _show_ollama_server_screen(self):
        self._clear()
        center = ctk.CTkFrame(self.main_container, fg_color="transparent")
        center.place(relx=0.5, rely=0.5, anchor="center")
        ...
        ctk.CTkLabel(
            center,
            text=self._ollama_status_detail,
            font=("Inter", 10),
        ...
        ctk.CTkButton(
            center,
            text="Retry Ollama Check",
            width=190,
            corner_radius=8,
            font=("Inter", 12, "bold"),
            fg_color=_T.accent,
            hover_color=_T.accent_hov,
            text_color="#FFFFFF",
            command=self._retry_ollama_check,
        ).pack()

    def _retry_ollama_check(self):
        result = check_ollama()
        self._ollama_installed = getattr(result, "ok", False)
        self._ollama_status_detail = getattr(result, "detail", "")
        value = getattr(result, "value", {}) or {}
        if isinstance(value, dict):
            self._ollama_server_running = bool(value.get("server_running", False))
    ...
    def _show_local_model_screen(self):
        ...
        ctk.CTkLabel(body, text="Choose a local model", font=("Inter", 20, "bold"), text_color=_T.text).pack(anchor="w", pady=(0, 6))
        ctk.CTkLabel(
            body,
            text=f"Recommended for this device: {self._recommended_model}",
            font=("Inter", 12),
            text_color=_T.dim,
        ).pack(anchor="w", pady=(0, 18))

        installed_section = ctk.CTkFrame(body, fg_color=_T.surface2, border_color=_T.border, border_width=1, corner_radius=10)
        installed_section.pack(fill="x", pady=(0, 14))

        ctk.CTkLabel(installed_section, text="Available on this device", font=("Inter", 14, "bold"), text_color=_T.text).pack(anchor="w", padx=18, pady=(16, 4))
        if self._installed_models:
            ...
            ctk.CTkOptionMenu(
                ...
                variable=self._installed_model_var,
                width=300,
                fg_color=_T.surface,
                button_color=_T.green,
                button_hover_color=_T.green,
                text_color=_T.text,
                font=("Inter", 12),
            ...
            ctk.CTkLabel(
                installed_section,
                text="No Ollama models are installed yet.",
                font=("Inter", 12),
                text_color=_T.dim,
        ...
        ctk.CTkLabel(install_section, text="Install a new model", font=("Inter", 14, "bold"), text_color=_T.text).pack(anchor="w", pady=(0, 8))
        self._install_model_var = ctk.StringVar(value=self._recommended_model)
        ctk.CTkOptionMenu(
            install_section,
            values=self._ordered_model_options(),
            variable=self._install_model_var,
            width=300,
            fg_color=_T.surface2,
            button_color=_T.accent,
            button_hover_color=_T.accent_hov,
            text_color=_T.text,
            font=("Inter", 12),
        ...
        ctk.CTkButton(
            ...
            hover_color=_T.accent_hov,
            text_color="#FFFFFF",
            command=self._install_selected_model,
        ).pack(anchor="w")
    ...
    def _start_ollama_install(self):
        ...
        def progress(msg: str):
            self.after(0, lambda: self._install_sub.configure(text=msg))
            
        def run():
            result = install_ollama(progress_callback=progress)
            if getattr(result, 'ok', False):
                self._ollama_installed = True
                self.after(400, self._retry_ollama_check)
            else:
                self.after(0, lambda: self._install_btn.configure(state="normal", text="Retry Installation"))
                self.after(0, lambda: self._install_sub.configure(text=getattr(result, 'detail', 'Error'), text_color=_T.danger))
    ...
    def _show_model_pull_screen(self):
        ...
        ctk.CTkLabel(center, text=f"Pulling {self._selected_local_model}", font=("Inter", 18, "bold"), text_color=_T.text).pack(pady=(0, 6))
    ...
    def _run_model_pull(self):
        def progress_ui_bridge(msg: str):
            # Parse text lines safely into fractional progress layout updates
            self.after(0, lambda: self._pull_sub.configure(text=msg))
            pct_match = re.search(r"(\d+)%", msg)
            if pct_match:
                pct = int(pct_match.group(1)) / 100
                self.after(0, lambda: self._model_bar.set(pct))

        success = pull_model(self._selected_local_model, progress_callback=progress_ui_bridge)
        if success:
            self.after(0, lambda: self._pull_sub.configure(text="Verifying model orchestration integrity…"))
            ok = verify_ollama(self._selected_local_model, progress_callback=progress_ui_bridge)
            if ok:
                self._save_local_config()
                self.after(0, lambda: self._model_bar.set(1.0))
                self.after(600, self._terminate_and_hand_off)
            else:
                self.after(0, lambda: self._pull_sub.configure(text="Model pulled but verification failed. Try restarting.", text_color=_T.danger))
        else:
            self.after(0, lambda: self._pull_sub.configure(text="Download failed. Check your internet connection.", text_color=_T.danger))

    def _save_local_config(self):
        _save_phil_config({
            "setup_complete": True,
            "mode": "local",
            "local_model": self._selected_local_model,
            "provider": None,
            "api_model": None,
            "api_key_env": None,
        })
    ...
    def _save_api_config(self):
        ...
        if not api_key:
            ...
            self._api_error.configure(text="API key cannot be empty.")
            return
